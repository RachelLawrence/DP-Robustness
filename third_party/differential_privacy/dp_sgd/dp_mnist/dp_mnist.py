...
def MnistInput_image(input_image, batch_size, randomize):
    image = tf.cast(tf.image.decode_png(input_image, channels=1),
                    tf.float32)
    image = tf.reshape(image, [IMAGE_SIZE * IMAGE_SIZE])
    image /= 255
    label = tf.cast(1, dtype=tf.int32)
    label = tf.reshape(label, [])
...
def Eval_no_softmax(mnist_data_file, network_parameters, num_testing_images,
                    randomize, load_path, save_mistakes=False):
    """Evaluate MNIST for a number of steps.
...
def build_mnist_model(load_path, network_parameters):
    g = tf.Graph()
    with g.as_default():
        sess = tf.get_default_session()

        # Create the basic Mnist model.
        inputs = tf.placeholder(tf.float32, (1, 28, 28, 1), name='image_in')
        logits, _, _ = utils.BuildNetwork(inputs, network_parameters)
        # logits = tf.nn.softmax(logits)
        logits = tf.identity(logits, name='mnist_out')
...
def Eval_one_no_softmax(image, network_parameters, num_testing_images,
    ...
    # Like for training, we need a session for executing the TensorFlow graph.
    sess = tf.get_default_session()
    mnist_graph = build_mnist_model(load_path, network_parameters)
    [logits] = tf.import_graph_def(mnist_graph, input_map={'image_in': image}, return_elements=['mnist_out:0'])

...
class CarliniL2:
    def __init__(self, sess, model, batch_size=1, confidence=CONFIDENCE,
        ...
        max_other_value = tf.reduce_max((1 - self.tlab) * self.output_tensor, 1)
        label_value = tf.reduce_sum((self.tlab) * self.output_tensor, 1)
        value_diff = tf.reshape(tf.subtract(max_other_value, label_value), [])
        def zeroval(): return tf.constant(100, dtype=tf.float32)
        def posval(): return tf.divide(1.0, value_diff)
